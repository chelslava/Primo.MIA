...
| **Database (ADO.NET)** | 16 | Операции с реляционными БД через ADO.NET. Запросы, транзакции, пакетные операции, DataTable. |
| **Database (MongoDB)** | 6 | NoSQL операции с MongoDB. Запросы, вставка, обновление, удаление, агрегация. |
| **Database (Redis)** | 10 | Кэширование и очереди через Redis. Строки, хэши, списки, счётчики. |
...
| **JsonDataTable** | 1 | Конвертация DataTable ↔ JSON без зависимостей. Три формата, выборка колонок, автотипы. |
