...
8. `Scalar` умеет отдавать типизированные выходы, `Query` возвращает список колонок, `NonQuery` умеет исполнять SQL-скрипты по `GO` batch, а `BulkInsert` поддерживает предварительную очистку таблицы и provider-safe quoting имён.
