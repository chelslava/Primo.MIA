...
namespace Primo.MIA
{
    ...
    public class ExcelCellRecalculateBack : PrimoComponentTO<ExcelCellRecalculate>
    {
        ...
        public ExcelCellRecalculateBack(IWFContainer container) : base(container)
        {
            ...
            sdkComponentHelp = @"Компонент ""Excel: Пересчёт ячейки""
Вычисляет новое имя ячейки Excel путём применения смещения по строкам и столбцам относительно начальной ячейки. Поддерживает однобуквенные и многобуквенные имена столбцов (A1, Z10, AA5, BC100 и т.д.).

Основные:
Начальная ячейка*: [String] Имя начальной ячейки в формате Excel (например, A1, B5, AA10).

Смещение:
Смещение по строкам: [Int32] Количество строк для сдвига относительно начальной ячейки (может быть отрицательным). По умолчанию 0.
Смещение по столбцам: [Int32] Количество столбцов для сдвига относительно начальной ячейки (может быть отрицательным). По умолчанию 0.
