// =============================================================================
// ElementHoverWithOffsetBack.cs — активность «Наведение с точным смещением».
//
// Наводит курсор на элемент с указанным смещением от центра элемента.
// Использует Actions API для точного позиционирования курсора.
//
// Используется для:
//   - Наведения на конкретную точку элемента
//   - Активации подменю и всплывающих подсказок
//   - Точного позиционирования для drag and drop
// =============================================================================
...
{
    /// <summary>
    /// Активность для наведения курсора на элемент с точным смещением.
    /// </summary>
    public class ElementHoverWithOffsetBack : PrimoComponentTO<ElementHoverWithOffset>
    {
        ...
        /// <summary>ID элемента для наведения.</summary>
        [LTools.Common.Model.Serialization.StoringProperty]
        [LTools.Common.Model.Studio.ValidateReturnScript(DataType = typeof(string))]
        [System.ComponentModel.Category(ActivityStrings.Category_Main),
        ...
        /// <summary>Смещение по X от центра элемента.</summary>
        [LTools.Common.Model.Serialization.StoringProperty]
        ...
        /// <summary>Смещение по Y от центра элемента.</summary>
        [LTools.Common.Model.Serialization.StoringProperty]
        [LTools.Common.Model.Studio.ValidateReturnScript(DataType = typeof(int))]
        [System.ComponentModel.Category(ActivityStrings.Category_Offset),
        ...
        public ElementHoverWithOffsetBack(IWFContainer container) : base(container)
        {
            sdkComponentName = "Элемент: Наведение с смещением";
            sdkComponentHelp =
                "Наводит курсор на элемент с точным смещением от центра.\n" +
                "\n" +
                "── Основные параметры ────────────────────────\n" +
                "ID сессии — идентификатор сессии браузера\n" +
                "ID элемента — идентификатор элемента (если уже найден)\n" +
                "Смещение X — смещение по горизонтали от центра (пиксели)\n" +
                "Смещение Y — смещение по вертикали от центра (пиксели)\n" +
                "\n" +
                ...
                "\n" +
                "ПРИМЕЧАНИЕ: Смещение 0,0 означает центр элемента.\n" +
            ...
            sdkProperties = new List<LTools.Common.Helpers.WFHelper.PropertiesItem>()
            {
                ...
                PropertyBuilder.Int("Prop_OffsetX", "Смещение X"),
                PropertyBuilder.Int("Prop_OffsetY", "Смещение Y"),
        ...
        public override ExecutionResult TimedAction(ScriptingData sd)
        {
            try
            {
                ...
                string resultMsg = $"[Наведение с смещением] Выполнено: смещение ({offsetX}, {offsetY})";
            ...
            catch (Exception ex)
            {
                ...
                {
                    IsSuccess = false,
                    ErrorMessage = $"Ошибка [Наведение с смещением]: {ex.Message}"
                };
            }
        }
