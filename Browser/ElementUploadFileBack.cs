...
namespace Primo.MIA
{
    ...
    public class ElementUploadFileBack : BrowserActivityBase<ElementUploadFile>
    {
        ...
        public bool Prop_VerifyFileExists
        {
            ...
            set { _propVerifyFileExists = value; InvokePropertyChanged(this, nameof(Prop_VerifyFileExists)); }
        }
            ...
            return SafeExecute(() =>
            {
                ...
                if (!string.IsNullOrWhiteSpace(locatorValue))
                {
                    ...
                    Logger.LogDebug(sdkComponentName, 
                        "Ожидание input[type=file] элемента: {0}='{1}', режим: {2}", 
                ...
                else if (!string.IsNullOrWhiteSpace(elementId))
                {
                    ...
                    // Проверяем кликабельность input[type=file] элемента из репозитория
                    if (!element.Displayed || !element.Enabled)
                    {
                        throw new ElementNotInteractableException(
                            $"Input[type=file] элемент с ID '{elementId}' не кликабельный (Displayed: {element.Displayed}, Enabled: {element.Enabled})");
                    }
