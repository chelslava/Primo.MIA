// =============================================================================
// ElementRightClickBack.cs — активность «Правый клик по элементу».
//
// Выполняет правый клик (контекстное меню) по элементу на странице.
// Использует Actions API для эмуляции правого клика мыши.
//
// Используется для:
//   - Вызова контекстного меню
//   - Копирования/вставки через меню
//   - Взаимодействия с элементами требующими правого клика
// =============================================================================
...
{
    /// <summary>
    /// Активность для правого клика по элементу на странице.
    /// </summary>
    public class ElementRightClickBack : PrimoComponentTO<ElementRightClick>
    {
        ...
        /// <summary>ID элемента для правого клика.</summary>
        [LTools.Common.Model.Serialization.StoringProperty]
        [LTools.Common.Model.Studio.ValidateReturnScript(DataType = typeof(string))]
        [System.ComponentModel.Category(ActivityStrings.Category_Main),
        ...
        public ElementRightClickBack(IWFContainer container) : base(container)
        {
            sdkComponentName = "Элемент: Правый клик";
            sdkComponentHelp =
                ...
                "\n" +
                "ПРИМЕЧАНИЕ: После правого клика обычно появляется\n" +
                "контекстное меню, с которым можно взаимодействовать.";
        ...
        public override ExecutionResult TimedAction(ScriptingData sd)
        {
            try
            {
                ...
                string resultMsg = !string.IsNullOrWhiteSpace(locatorValue)
                    ? $"[Правый клик] Выполнен: {this.Prop_LocatorType}={locatorValue}"
                    : $"[Правый клик] Выполнен: {elementId}";
            ...
            catch (Exception ex)
            {
                ...
                {
                    IsSuccess = false,
                    ErrorMessage = $"Ошибка [Правый клик]: {ex.Message}"
                };
            }
        }
