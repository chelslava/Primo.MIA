// =============================================================================
// ElementClickAndHoldBack.cs — активность «Клик с удержанием».
//
// Нажимает и удерживает кнопку мыши на элементе указанное время.
// Использует Actions API для эмуляции удержания кнопки мыши.
//
// Используется для:
//   - Drag and Drop операций (начальная фаза)
//   - Активации элементов требующих длительного нажатия
//   - Тестирования поведения при удержании
// =============================================================================
...
{
    /// <summary>
    /// Активность для клика с удержанием кнопки мыши на элементе.
    /// </summary>
    public class ElementClickAndHoldBack : PrimoComponentTO<ElementClickAndHold>
    {
        ...
        /// <summary>ID элемента для клика с удержанием.</summary>
        [LTools.Common.Model.Serialization.StoringProperty]
        [LTools.Common.Model.Studio.ValidateReturnScript(DataType = typeof(string))]
        [System.ComponentModel.Category(ActivityStrings.Category_Main),
        ...
        /// <summary>Длительность удержания (мс).</summary>
        [LTools.Common.Model.Serialization.StoringProperty]
        [LTools.Common.Model.Studio.ValidateReturnScript(DataType = typeof(int))]
        [System.ComponentModel.Category(ActivityStrings.Category_Main),
        ...
        public string Prop_HoldDuration
        {
            ...
            set { _propHoldDuration = value; InvokePropertyChanged(this, "Prop_HoldDuration"); }
        }

        // ── Конструктор ────────────────────────────────────────────────

        public ElementClickAndHoldBack(IWFContainer container) : base(container)
        {
            sdkComponentName = "Элемент: Клик с удержанием";
            sdkComponentHelp =
                "Нажимает и удерживает кнопку мыши на элементе.\n" +
                "\n" +
                "── Основные параметры ────────────────────────\n" +
                "ID сессии — идентификатор сессии браузера\n" +
                "ID элемента — идентификатор элемента (если уже найден)\n" +
                "Длительность удержания — время удержания кнопки (мс)\n" +
                "\n" +
                ...
                "\n" +
                "ПРИМЕЧАНИЕ: После удержания кнопка мыши автоматически\n" +
                "отпускается. Для ручного управления используйте Actions API.";
        ...
        public override ExecutionResult TimedAction(ScriptingData sd)
        {
            try
            {
                ...
                // Получение длительности удержания
                string durationStr = GetPropertyValue<string>(this.Prop_HoldDuration, "Prop_HoldDuration", sd) ?? "1000";
                int duration = int.TryParse(durationStr, out int d) ? d : 1000;
                if (duration < 0) duration = 0;

                // Выполнение клика с удержанием
                SeleniumHelper.ClickAndHold(driver, element, duration);

                string resultMsg = !string.IsNullOrWhiteSpace(locatorValue)
                    ? $"[Клик с удержанием] Выполнен: {this.Prop_LocatorType}={locatorValue}, удержание {duration}мс"
                    : $"[Клик с удержанием] Выполнен: {elementId}, удержание {duration}мс";
            ...
            catch (Exception ex)
            {
                ...
                {
                    IsSuccess = false,
                    ErrorMessage = $"Ошибка [Клик с удержанием]: {ex.Message}"
                };
            }
        }
