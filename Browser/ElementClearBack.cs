// =============================================================================
// ElementClearBack.cs — активность «Очистить поле».
//
// Очищает содержимое поля ввода.
// Используется перед вводом нового текста или для сброса формы.
//
// ВАЖНО: Работает только с полями ввода (input, textarea).
// =============================================================================
...
{
    /// <summary>
    /// Активность для очистки поля ввода.
    /// </summary>
    public class ElementClearBack : PrimoComponentTO<ElementClear>
    {
        ...
        public ElementClearBack(IWFContainer container) : base(container)
        {
            sdkComponentName = "Элемент: Очистить поле";
            sdkComponentHelp =
                "Очищает содержимое поля ввода.\n" +
                "\n" +
                "── Основные параметры ────────────────────────\n" +
                ...
                "\n" +
                "ПРИМЕЧАНИЕ: Работает с input и textarea элементами.\n" +
        ...
        public override ExecutionResult TimedAction(ScriptingData sd)
        {
            try
            {
                ...
                string resultMsg = !string.IsNullOrWhiteSpace(locatorValue)
                    ? $"[Очистить поле] Элемент очищен: {this.Prop_LocatorType}={locatorValue}"
                    : $"[Очистить поле] Элемент {elementId} очищен";
            ...
            catch (Exception ex)
            {
                ...
                {
                    IsSuccess = false,
                    ErrorMessage = $"Ошибка [Очистить поле]: {ex.Message}"
                };
            }
        }
