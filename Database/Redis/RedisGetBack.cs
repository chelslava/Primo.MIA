...
namespace Primo.MIA
{
    public class RedisGetBack : PrimoComponentTO<RedisGet>
    {
        ...
        [LTools.Common.Model.Serialization.StoringProperty]
        [LTools.Common.Model.Studio.ValidateReturnScript(DataType = typeof(string))]
        [System.ComponentModel.Category(ActivityStrings.Category_Output), System.ComponentModel.DisplayName(ActivityStrings.Field_RedisValue)]
        ...
        public RedisGetBack(IWFContainer container) : base(container)
        {
            sdkComponentName = ActivityStrings.Activity_RedisGet;
            sdkComponentHelp = @"Компонент ""Redis: Получить значение""
Получает строковое значение по ключу из Redis. Если ключ отсутствует — возвращает пустую строку и Exists = False.

Основные:
Строка подключения*: [String] Redis connection string (например, localhost:6379).
...
Ключ существует: [Boolean] True, если ключ найден в Redis.";
            ...
            sdkProperties = new List<LTools.Common.Helpers.WFHelper.PropertiesItem>()
            {
                ...
                PropertyBuilder.Variable<string>("Prop_Value", "Полученное значение"),
        ...
        public override ExecutionResult TimedAction(ScriptingData sd)
        {
            try
            {
                ...
                SetVariableValue(Prop_Value, value.HasValue ? (string)value : string.Empty, sd);
                SetVariableValue(Prop_Exists, value.HasValue, sd);

                return new ExecutionResult { IsSuccess = true, SuccessMessage = value.HasValue ? $"Ключ найден: {key}" : $"Ключ отсутствует: {key}" };
            }
            catch (Exception ex)
            {
                return new ExecutionResult { IsSuccess = false, ErrorMessage = $"Ошибка Redis GET: {ex.Message}" };
            }
        }
