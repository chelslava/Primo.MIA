...
namespace Primo.MIA
{
    ...
    public class DatabaseTransactionRollbackBack : PrimoComponentTO<DatabaseTransactionRollback>
    {
        ...
        public DatabaseTransactionRollbackBack(IWFContainer container) : base(container)
        {
            ...
            sdkComponentHelp = @"Компонент ""Database: Откатить транзакцию""
Отменяет все изменения, накопленные в транзакции, и закрывает её. Работает с явно переданным transactionId или с текущим ambient-контекстом модуля.
